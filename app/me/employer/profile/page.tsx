...
export default async function WorkspaceProfilePage({ searchParams }: ProfilePageProps) {
  ...
  const [user, postedJobs, savedJobs, receivedApplicationsCount, applications] = await Promise.all([
  ...
  const stats = [
    ...
    { label: locale === "ro" ? "Aplicatii primite" : "Received applications", value: receivedApplicationsCount },
  ];
  ...
  const securityMessages: Record<string, string> = {
    ...
    cv_not_available_for_employer:
      locale === "ro"
        ? "CV Builder nu este disponibil pentru conturile de angajator."
        : "CV Builder is not available for employer accounts.",
    resume_not_available_for_employer:
      locale === "ro"
        ? "CV Builder nu este disponibil pentru conturile de angajator."
        : "CV Builder is not available for employer accounts.",
